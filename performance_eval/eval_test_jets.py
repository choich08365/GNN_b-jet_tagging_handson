...
def eval_jets_on_test_set(model):
    ...
    test_ds = uproot.open('data/test/test_data.root') #FileNotFoundError: file not foound 'data/jets_data_all/test/test_data.root' # 'data/jets_data_all/test/test_data.root' -> 'data/test/test_data.root'
    jet_df = test_ds['tree'].pandas.df(['jet_flav', 'trk_vtx_index'], flatten=False) #AttributeError: 'Model_TTree_v20' object has no attribute 'pandas' # uproot3 version
...
def _predict_on_test_set(model):
    test_ds = JetGraphDataset('test')#, random_permutation=False) #TypeError: __init__() got an unexpected keyword argument 'random_permutation'
    model.eval()
    ...
    for tracks_in_jet in range(2, np.amax(n_tracks)+1):
        ...
        input_batch = torch.stack([torch.tensor(test_ds[i][0]) for i in trk_indxs])  # shape (B, N_i, 10) #TypeError: expected Tensor as element 0 in argument 0, but got numpy.ndarray #test_ds[i][0] -> torch.tensor(test_ds[i][0])
        ...
        edge_scores[:, np.arange(tracks_in_jet), np.arange(tracks_in_jet)] = 1. #NameError: name 'N' is not defined # N -> len(tracks_in_jet) # selecting diagonal terms in each tensor
