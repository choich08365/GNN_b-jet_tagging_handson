...
class DiagOffdiagMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(f"DEBUG:DiagOffdiagMLP.forward() x.shape={x.shape}")
        return self.conv_offdiag(x) #RuntimeError: Given groups=1, weight of size [256, 25, 1, 1], expected input[2074, 20, 4, 4] to have 25 channels, but got 20 channels instead
